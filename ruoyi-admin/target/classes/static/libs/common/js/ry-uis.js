...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            adds: function (id) {
                var url = $.common.isEmpty(id) ? $.table._option.createUrls : $.table._option.createUrls.replace("{id}", id);
                $.modal.open("添加" + $.table._option.modalName, url);
            },
            // 修改信息
            edit: function (id) {
            ...
            edits: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.updateUrlx.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            editTrees: function () {
                ...
                var url = $.table._option.updateUrls.replace("{id}", row[$.table._option.uniqueId]);
                $.modal.open("修改" + $.table._option.modalName, url);
            },
        ...
        tree: {
            ...
            notAllowParents: function (_tree) {
                ...
                for (var i = 0; i < nodes.length; i++) {
                    if (nodes[i].level == 0) {
                        $.modal.msgError("不能选择（" + nodes[i].name + "）");
                        return false;
                    }
                    if (nodes[i].isParent) {
                        $.modal.msgError("不能选择（" + nodes[i].name + "）");
            ...
            Parents: function (_tree) {
                ...
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    // }
                    if (!nodes[i].isParent) {
                        $.modal.msgError("只能选择区域经理（" + nodes[i].name + "）");
                        return false;
                    }
                }
                return true;
            ...
            // 只能选择道路树状图
            notAllowroadLevel: function (_tree) {
                var nodes = _tree.getSelectedNodes();
                for (var i = 0; i < nodes.length; i++) {
                    if (nodes[i].level == nodes.length + 1) {
                        $.modal.msgError("不能选择最后层级节点（" + nodes[i].name + "）");
